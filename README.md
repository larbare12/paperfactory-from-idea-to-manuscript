...
> 全流程论文写作辅助 Claude Code Plugin。
> M0 仪表盘 + M1–M9 九阶段模块 + citation-search 独立子 skill,
> 覆盖选题诊断、文献管理(含 Tier 0 反幻觉校验 + venue quality 过滤)、
> 实验设计、结构规划、论证设计、写作辅助、投稿前总检、同行评审仿真、
> 合规与伦理检查。
...
把整个 plugin 目录放到 Claude Code 的 plugin 路径(参见 Claude Code 文档),或在
本目录直接启动 Claude Code session。
...
| 阶段 | 用法 |
|---|---|
| 想找文献 / 验证引用 / 查 venue 质量 | `@citation-search` |
| 评估 idea 可行性 | `@m1-topic` |
| 文献综述 / bib 整理 | `@m2-literature` |
| 实验设计 | `@m3-experiment` |
| IMRAD 大纲 | `@m4-structure` |
| 论证骨架 / DA 红队 | `@m5-argument` |
| 写正文 / 段落润色 | `@m6-writing` |
| 投稿前总检 | `@m7-final-check` |
| 同行评审仿真 | `@m8-peer-review` |
| 合规 + AI 披露 | `@m9-compliance-check` |
| 看项目状态 | `@m0-dashboard` |
...
### citation-assistant
**联网文献检索、期刊质量评估、BibTeX 生成**能力的种子来自
[**citation-assistant**](https://github.com/ZhangNy301/citation-assistant)。
`skills/citation-search/scripts/` 下的 `paper_search.sh` / `venue_lookup.sh` /
`author_info.sh` / `doi2bibtex.sh` 以及 CCF 与影响因子数据库均基于该项目改造。

### academic-research-skills (ARS)
**M8/M9 模块设计、`reference/` 与 `templates/` 下 62 个参考文件和模板**
